...
Classify stream reaches where modeled channel width does not exist using a stream order based approach. Approach taken from 2024 Boundary TSA. Source: `file:///G:/!Workgrp/Analysts/!Project/TSA_Projects/active/KootenayLake_TSA13/TSR4/AnalysisReport/TSA13_TSR4_AnalysisReport/03-data-analysis.html#riparian-analysis`
...
run_sql_r(query, conn_list)
query <- "UPDATE whse_sp.modelled_habitat_potential fishy SET tsa = tsa.tsa
FROM
whse_sp.tsa_boundaries_2020 tsa
WHERE
ST_Intersects(tsa.geom, fishy.line_center_point)
;"
run_sql_r(query, conn_list)
```
...
END,
	...
	WHEN 
	  ...
	  THEN 'stream order >= 4 and fish presence or community watershed'
	WHEN
	  stream_order = 3 AND (fish_habitat != 'NON FISH HABITAT' OR community_watershed) 
	  THEN 'stream order = 3 and fish presence or community watershed'
	WHEN
	  stream_order = 2 AND (fish_habitat != 'NON FISH HABITAT' OR community_watershed) 
	  THEN 'stream order = 2 and fish presence or community watershed'
	WHEN
	  stream_order = 1 AND (fish_habitat != 'NON FISH HABITAT' OR community_watershed) 
	  THEN 'stream order = 1 and fish presence or community watershed'
	WHEN
	  stream_order > 1 AND (fish_habitat = 'NON FISH HABITAT' OR NOT community_watershed) 
	  THEN 'stream order > 1 and fish absence or not community watershed'
	WHEN
	  stream_order = 1 AND (fish_habitat = 'NON FISH HABITAT' OR NOT community_watershed) 
	  THEN 'stream order = 1 and fish absence or not community watershed'
END,
	riparian_data_source = 'Fish Habitat Accessibility MODEL; WHSE_WATER_MANAGEMENT.WLS_COMMUNITY_WS_PUB_SVW'
WHERE
  riparian_class IS NULL;"
run_sql_r(query, conn_list)
```
...
query <- "ALTER TABLE whse_sp.modelled_habitat_potential DROP COLUMN IF EXISTS inside_double_sided_river;"
run_sql_r(query, conn_list)
query <- "ALTER TABLE whse_sp.modelled_habitat_potential ADD COLUMN inside_double_sided_river boolean default FALSE;"
run_sql_r(query, conn_list)

query <- "UPDATE whse_sp.modelled_habitat_potential fishy SET inside_double_sided_river = TRUE
FROM
  whse_sp.fwa_rivers_poly riv
WHERE
...
query <- "UPDATE whse_sp.fwa_wetlands_poly SET riparian_class = 'W1', riparian_class_reason = 'wetland is greater than 5 ha in size' WHERE feature_area_sqm/10000 > 5"
run_sql_r(query, conn_list)
```
...
SET 
	riparian_class = 'W2', 
	riparian_class_reason = 'wetland is not less than 1 ha and not more than 5 ha in size and is in one of the following biogeoclimatic zones or subzones' 
FROM 
	w2
WHERE 
...
SET 
	riparian_class = 'W3', 
  riparian_class_reason = 'wetland is not less than 1 ha and not more than 5 ha in size and is in a biogeoclimatic zone or subzone other than one referred to in paragraph (b)'
WHERE 
...
SET 
	riparian_class = 'W4', 
	riparian_class_reason = 'not less than 0.25 ha and less than 1 ha in size and is in a biogeoclimatic zone or subzone referred to in paragraph (b) (i), (ii) or (iii)' 
FROM 
	w4
WHERE 
...
SET 
	riparian_class = 'L2', 
	riparian_class_reason = 'lake is not less than 1 ha and not more than 5 ha in size and is located in a biogeoclimatic zones or subzone' 
FROM 
	w2
WHERE 
	w2.waterbody_poly_id = wet.waterbody_poly_id"
...
query <- "UPDATE 
		  ...
		  SET 
		  		riparian_class = 'L3', 
				riparian_class_reason = 'lake is not less than 1 ha and not more than 5 ha in size and is in a biogeoclimatic zone or subzone other than one referred to in paragraph (c)'
		  WHERE 
		  		feature_area_sqm/10000 <= 5 
		  AND 
		  		feature_area_sqm/10000 > 1 
...
Buffers were determined from the regulations found [here](https://www.bclaws.gov.bc.ca/civix/document/id/complete/statreg/14_2004#division_d2e9829)  

> _"Haida Gwaii has different riparian setbacks than FPPR. The Great Bear Rainforest also has different setbacks because of a higher level order and FLPs that are currently in progress." ~ Lisa Nordin_

__Restrictions in a riparian management zones__

> _"52   (1)A holder of a minor tenure who fells trees in a cutblock within a riparian management zone of a class described in Column 1 must ensure that:  
...
(b)the standing trees are reasonably representative of the physical structure of the riparian management zone, as it was before harvesting:"_

```{r echo = FALSE, results = 'asis'}

riparian_class <- c("S1-A or S1-B stream", "S2 stream", "S3 stream", "S4 stream", "S5 stream", "S6 stream", "All classes of wetlands or lakes")
basal_area <- c("≥ 20", "≥ 20", "≥ 20", "≥ 10", "≥ 10", "Not applicable", "≥ 10")

# Combine lists into a data.frame
df <- data.frame(
  `Riparian Class` = riparian_class, 
  `Basal Area to be Retained Within Riparian Management Zone (%)` = basal_area,
  check.names = FALSE
  )

kable(df, caption='Basal Area to be Retained Within Riparian Management Zone')
...
basal_area <- c("≥ 20", "≥ 20", "≥ 20", "≥ 20", "≥ 10", "≥ 10", "Not applicable")
applied_riparian_buffers <- c(20, 54, 34, 24, 3, 3, 0)
...
basal_area <- c("≥ 10", "≥ 10", "≥ 10", "≥ 10", "≥ 10")
applied_riparian_buffers <- c(14, 12, 3, 3, 14)
...
basal_area <- c("≥ 10", "≥ 10", "≥ 10", "≥ 10", "≥ 10")
applied_riparian_buffers <- c(0, 10, 12, 3, 3)
...
query <- "UPDATE whse_sp.fwa_lakes_poly SET riparian_buffer_width_m = CASE 
	...
	WHEN riparian_class = 'L2'  THEN 12
	WHEN riparian_class = 'L3'  THEN 3
	WHEN riparian_class = 'L4'  THEN 3
	END;"
run_sql_r(query, conn_list)
...
query <- "UPDATE whse_sp.fwa_rivers_poly SET riparian_buffer_width_m = CASE 
	WHEN riparian_class = 'S1A' THEN 20
	WHEN riparian_class = 'S1B' THEN 54
	WHEN riparian_class = 'S2'  THEN 34
	WHEN riparian_class = 'S3'  THEN 24
	WHEN riparian_class = 'S4'  THEN 3
	WHEN riparian_class = 'S5'  THEN 3
	WHEN riparian_class = 'S6'  THEN 0
	END;"
run_sql_r(query, conn_list)
...
query <- "UPDATE whse_sp.fwa_wetlands_poly SET riparian_buffer_width_m = CASE 
	WHEN riparian_class = 'W1'  THEN 14
	WHEN riparian_class = 'W2'  THEN 12
	WHEN riparian_class = 'W3'  THEN 3
	WHEN riparian_class = 'W4'  THEN 3
	WHEN riparian_class = 'W5'  THEN 14
	END;"
run_sql_r(query, conn_list)
query <- "UPDATE whse_sp.fwa_wetlands_poly SET riparian_buffer_geom = ST_Buffer(geom, riparian_buffer_width_m) WHEN riparian_buffer_width_m IS NOT NULL;"
run_sql_r(query, conn_list)
...
query <- "
CREATE TABLE whse_sp.riparian_buffered_areas AS
WITH all_buffers_appended AS (
  SELECT riparian_buffer_geom FROM whse_sp.modelled_habitat_potential WHERE riparian_class IS NOT NULL
  UNION ALL
  SELECT riparian_buffer_geom FROM whse_sp.fwa_wetlands_poly WHERE riparian_class IS NOT NULL
  UNION ALL
  SELECT riparian_buffer_geom FROM whse_sp.fwa_lakes_poly WHERE riparian_class IS NOT NULL
  UNION ALL
  SELECT riparian_buffer_geom FROM whse_sp.fwa_rivers_poly WHERE riparian_class IS NOT NULL
) 
SELECT
...
<details>
  ...
  <br>
Write out lakes to a FileGDB
```{r eval = FALSE}
  
  sql_query <- paste0('-sql ', glue::double_quote("SELECT waterbody_poly_id::int, watershed_group_id::int, waterbody_type, gnis_name_1, fwa_watershed_code, local_watershed_code, watershed_group_code, left_right_tributary, feature_area_sqm, feature_length_m, geom, riparian_class_reason, riparian_class FROM whse_sp.fwa_lakes_poly"))
  ogr_conn <- paste0('PG:', glue::double_quote("dbname='prov_data' host='localhost' user='postgres'"))
  cmd_streams <- paste0('ogr2ogr -f ', glue::double_quote("FileGDB"), glue(' data/clean/test.gdb {ogr_conn} -nlt MULTIPOLYGON -nln table_nam2 {sql_query} -update'))
  system(cmd_streams)
```
</details>
