...
## Goal is to filter out all vri polygons that have a site index less than 5th percentile within mgmt unit that was >= 30 cutblocks
## if the mgmt unit has < 30 cutblocks, use a the TSA it resides within
...
WITH tsa_cc AS (
	...
	LEFT JOIN thlb_proxy.veg_comp_lyr_r1_poly_2016_gr_skey vri_key on vri_key.gr_skey = tsa_key.gr_skey
	LEFT JOIN (SELECT CASE WHEN site_index = 0 THEN NULL ELSE site_index END AS site_index, bclcs_level_1, pgid FROM thlb_proxy.veg_comp_lyr_r1_poly_2016) vri ON vri.pgid = vri_key.pgid
	LEFT JOIN thlb_proxy.tfl_integrated2016_gr_skey vritfl_key ON vritfl_key.gr_skey = vri_key.gr_skey
	LEFT JOIN (SELECT CASE WHEN site_index = 0 THEN NULL ELSE site_index END AS site_index, pgid FROM thlb_proxy.tfl_integrated2016) vritfl ON vritfl_key.pgid = vritfl.pgid
	WHERE
		harvest_year >= 2017
    GROUP BY
		mu_look.man_unit
...
WITH tfl_cc AS (
  ...
  LEFT JOIN whse.veg_consolidated_cut_blocks_sp cc ON cc.pgid = cc_key.pgid
  LEFT JOIN thlb_proxy.veg_comp_lyr_r1_poly_2016_gr_skey vri_key on vri_key.gr_skey = tfl_key.gr_skey
  LEFT JOIN (SELECT CASE WHEN site_index = 0 THEN NULL ELSE site_index END AS site_index, bclcs_level_1, pgid FROM thlb_proxy.veg_comp_lyr_r1_poly_2016) vri ON vri.pgid = vri_key.pgid
  LEFT JOIN thlb_proxy.tfl_integrated2016_gr_skey vritfl_key ON vritfl_key.gr_skey = vri_key.gr_skey
  LEFT JOIN (SELECT CASE WHEN site_index = 0 THEN NULL ELSE site_index END AS site_index, pgid FROM thlb_proxy.tfl_integrated2016) vritfl ON vritfl_key.pgid = vritfl.pgid
  WHERE
...
FROM
  ...
  LEFT JOIN (SELECT CASE WHEN site_index = 0 THEN NULL ELSE site_index END as site_index, bclcs_level_1, pgid FROM thlb_proxy.veg_comp_lyr_r1_poly_2016) vri USING (pgid)
  LEFT JOIN thlb_proxy.tfl_integrated2016_gr_skey vritfl_key ON vritfl_key.gr_skey = vri_key.gr_skey
  LEFT JOIN (SELECT CASE WHEN site_index = 0 THEN NULL ELSE site_index END as site_index, pgid FROM thlb_proxy.tfl_integrated2016) vritfl ON vritfl_key.pgid = vritfl.pgid
...
GROUP BY
    ...
    cc_count.cc_count
      ...
      cc_count
        FROM
      thlb_proxy.tsa_link_tfl_manlic
...
missing_tfl_abt a
JOIN
whse.mu_lookup_table_im mu ON a.man_unit = mu.man_unit
JOIN
thlb_proxy.tsa_5p_site_index_cc tsa_5p ON tsa_5p.man_unit = a.tsa
UNION ALL
SELECT
        man_unit,
        forest_file_id,
...
FROM
        thlb_proxy.tsa_5p_site_index_cc
) 
SELECT * FROM final_tbl order by man_unit")
run_sql_r(query, conn_list)
query <- glue("DROP TABLE IF EXISTS {dst_schema}.tsa_5p_site_index_cc")
