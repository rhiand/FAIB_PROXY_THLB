...
Based on expert guidance, the determination of non-merchantable stands involved summarizing the site index from the 2016 Vegetation Resource Inventory (VRI) within all harvested openings from 2017 onward, across each TSA and TFL. Selecting the site index from the year preceding the harvest activity (e.g., 2016 for openings harvested in 2017 or later) was intended to reflect site productivity before recent harvesting, providing a more accurate representation of stand potential.

The 2016 VRI dataset was selected as it is the earliest available year with TFL site index data, ensuring the largest possible sample size of harvested openings. A key objective of the THLB proxy approach is to model THLB within TFLs and other area-based tenures (e.g., community forests, woodlots). Therefore, ensuring the inclusion of TFL site index data was a priority. 

Following expert advice, the 5th percentile of the site index distribution was chosen as the threshold for non-merchantability. VRI stands in 2023 falling below this threshold were classified as 'non-merchantable.' To recap, the following steps were performed for each TSA and TFL:
...
The 2016 VRI dataset was selected as it is the earliest available year with TFL site index data, ensuring the largest possible sample size of harvested openings. A key objective of the THLB proxy approach is to model THLB within TFLs and other area-based tenures (e.g., community forests, woodlots). Therefore, ensuring the inclusion of TFL species data was a priority. 

The analysis looked at, for each TSA, comparing the percent species harvested since 2017, relative to all area harvested since 2017 with percent existance on the landbase, relative to the area of the whole landbase (i.e. TSA area). A barchart was created illustrating the breakdown and can be found within the github repository here: 
```
data\analysis\non_commercial_species_TSA_bar_graphs_percent_occurrence_2024_12_12.pdf
```


reviewing the harvest history since 2017 of all species within each TSA. Non-commercial species are leading species  tree species An analysis was performed to 
